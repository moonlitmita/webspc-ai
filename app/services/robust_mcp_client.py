...
class RobustMultiServerMCPClient:
    ...
    async def validate_and_invoke_tool(self, tool_name: str, args: Dict[str, Any]) -> Any:
        """
        在调用工具前验证参数，避免因参数格式问题导致的错误
        """
        # 验证参数类型，确保字符串类型的参数确实是字符串
        validated_args = {}
        for key, value in args.items():
            if isinstance(value, (dict, list)):
                # 如果是复杂类型，转换为JSON字符串
                validated_args[key] = json.dumps(value, ensure_ascii=False)
            elif value is None:
                validated_args[key] = ""
            elif isinstance(value, bool):
                # 将布尔值转换为字符串
                validated_args[key] = str(value).lower()
            elif isinstance(value, (int, float)):
                # 将数字转换为字符串
                validated_args[key] = str(value)
            else:
                # 确保所有值都是字符串或其他基本类型
                validated_args[key] = str(value) if value is not None else ""
        ...
        # 调用工具
        try:
            result = await target_tool.ainvoke(validated_args)
            return result
        except Exception as e:
