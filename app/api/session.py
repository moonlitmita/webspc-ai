...
def end_session(
    conversation_id: str = Query(..., description="要结束的会话 ID（必填）"),
    user_id: str = Depends(verify_token)
):
    """
    结束指定会话。
    前端必须传入 conversation_id，后端不会自动查找或创建会话。
    """
    try:
        # ---- 清理活跃会话 ----
...
def get_session_history(user_id: str = Depends(verify_token)):
    """获取用户的历史会话记录（排除活跃会话）"""
    try:
        # 获取 Redis 中所有匹配用户 ID 的会话键
        pattern = f"session:{user_id}:*"
        session_keys = session_redis_client.keys(pattern)

        # 获取活跃会话 ID 列表（只保留 conversation_id 部分）
        ...
        for key in session_keys:
            ...
            if history.messages:
                ...
                # 获取最后一条消息的时间作为会话时间
                # 注意：由于 RedisChatMessageHistory 没有直接存储时间戳，
...
def get_session_detail(
    conversation_id: str = Query(..., description="会话 ID（必填）"),
    user_id: str = Depends(verify_token)
):
    """获取会话详情，前端必须传入 conversation_id"""
    try:
        history = get_redis_session_history(user_id, conversation_id)
        return {
            "code": 200,
...
def delete_session_history(
    conversation_id: str = Query(..., description="要删除的会话 ID"),
    user_id: str = Depends(verify_token)
):
    """
    删除指定的历史会话
...
def delete_multiple_session_history(
    conversation_ids: list[str] = Query(..., description="要删除的会话 ID 列表"),
    user_id: str = Depends(verify_token)
):
    """
    批量删除历史会话
